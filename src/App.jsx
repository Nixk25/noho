...
const LOCATIONS = {
  ...
  // ============ VOJTĚCHOV - VTE, 11 POHLEDŮ (body 6 a 10 záměrně vynechané) ============
  // POZOR: imgTop/imgLeft i defaultRotation jsou PŘIBLIŽNÉ (placeholder) — doladit ručně.
  // defaultRotation = azimut k větrníkům (Vojtěchov, centrálně u Hlinska); 0=sever, 90=východ.
  // Body 1, 3, 4 jsou "nepozorovatelné" → jen jedna fotka (panorama = prekazka, přepínač nic nemění).
  // Názvy obcí jsou MŮJ ODHAD z mapy — ověřit/přepsat podle klienta.
  vojtechov: {
    baseUrl: "https://noho.b-cdn.net/vizualizace%20fotky/vojtechov",
    photoPattern: { normal: "panorama", xray: "prekazka" },
    mapUrl:
      "https://noho.b-cdn.net/vizualizace%20fotky/vojtechov/groundView_vojtechovv4.webp",
    mapZoom: 1.0,
    markers: [
      { id: 1, photo: 1, imgTop: 64, imgLeft: 34, defaultRotation: 47, rotationSpeed: 3, singlePhoto: true, name: "1. Vítanov" },
      { id: 2, photo: 2, imgTop: 67, imgLeft: 55, defaultRotation: 356, rotationSpeed: 3, name: "2. Kameničky" },
      ...
      { id: 13, photo: 13, imgTop: 19, imgLeft: 34, defaultRotation: 186, rotationSpeed: 3, name: "10. Včelákov" },
    ],
  },
...
function App() {
  ...
  // Instance 360° prohlížeče (pro ovládání šipkami a zoomem)
  const viewer360 = useRef(null);

  // Plynulé otáčení 360° pohledu při držení šipky
  const startPan360 = (direction) => {
    if (panInterval.current || !viewer360.current) return;
    const step = direction === "left" ? -0.03 : 0.03;
    const rotate = () => {
      const v = viewer360.current;
      if (!v) return;
      const pos = v.getPosition();
      v.rotate({ yaw: pos.yaw + step, pitch: pos.pitch });
    };
    rotate();
    panInterval.current = setInterval(rotate, 16);
  };
          ...
          {config.is360 ? (
            <>
              ...
              <div className="zoom-controls">
                <button
                  className="zoom-btn zoom-btn-large"
                  onClick={() => viewer360.current?.zoomIn()}
                  title="Přiblížit"
                >
                  <svg
                    viewBox="0 0 24 24"
                ...
                <button
                  className="zoom-btn zoom-btn-small"
                  onClick={() => viewer360.current?.zoomOut()}
                  title="Oddálit"
                >
                  <svg
                    viewBox="0 0 24 24"
              ...
              {/* Navigační šipky - otáčejí 360° pohledem */}
              <button
                className="nav-arrow nav-arrow-left"
                onMouseDown={() => startPan360("left")}
                onMouseUp={stopPan}
